...
Grad::Grad(f64 _x, f64 _y, f64 _z, f64 _w): x(_x), y(_y), z(_z), w(_w) { }

Grad::Grad(f64 x, f64 y, f64 z) : Grad(x, y, z, 0) {}
...
f64 SimplexNoise::F2 = 0.5 * (sqrt(3.0) - 1.0);
f64 SimplexNoise::G2 = (3.0 - sqrt(3.0)) / 6.0;
f64 SimplexNoise::F4 = (sqrt(5.0) - 1.0) / 4.0;
f64 SimplexNoise::G4 = (5.0 - sqrt(5.0)) / 20.0;
f64 SimplexNoise::F3 = 1.0 / 3.0;
f64 SimplexNoise::G3 = 1.0 / 6.0;

// 2D simplex noise
f64 SimplexNoise::noise(vec2 p) {
	f64 xin = p.x;
	f64 yin = p.y;

	f64 n0, n1, n2; // Noise contributions from the three corners
					// Skew the input space to determine which simplex cell we're in
	f64 s = (xin + yin) * F2; // Hairy factor for 2D
	i32 i = fastfloor(xin + s);
	i32 j = fastfloor(yin + s);
	f64 t = (i + j) * G2;
	f64 X0 = i - t; // Unskew the cell origin back to (x,y) space
	f64 Y0 = j - t;
	f64 x0 = xin - X0; // The x,y distances from the cell origin
	f64 y0 = yin - Y0;
...
f64 SimplexNoise::noise(f64 p) {
	return noise(vec2((f32)p, 0));
}

f64 SimplexNoise::noise(f64 p, i32 octaves, f64 persistance, f64 lacunarity) {
	f64 freq = 1, amp = 1;
	f64 y = 0;
	for (i32 i = 0; i < octaves; i++) {
		y += (noise(p * freq) + 1) / 2 * amp;
		amp *= persistance;
		freq *= lacunarity;
	}
	return y / octaves;
...
f64 SimplexNoise::noise(vec2 p, i32 octaves, f64 persistance, f64 lacunarity) {
	f64 freq = 1, amp = 1;
	f64 z = 0;
	for (i32 i = 0; i < octaves; i++) {
		z += (noise(p * (f32)freq) + 1) / 2 * amp;
		amp *= persistance;
		freq *= lacunarity;
	}
	return z;
...
f64 SimplexNoise::noise(vec3 p, i32 octaves, f64 persistance, f64 lacunarity) {
	f64 freq = 1, amp = 1;
	f64 w = 0;
	for (i32 i = 0; i < octaves; i++) {
		w += (noise(p * (f32)freq)
...
f64 SimplexNoise::noise(vec4 p, i32 octaves, f64 persistance, f64 lacunarity) {
	f64 freq = 1, amp = 1;
	f64 w = 0;
	for (i32 i = 0; i < octaves; i++) {
		w += (noise(p * (f32)freq)
...
i32 SimplexNoise::fastfloor(f64 x) {
	i32 xi = (i32)x;
	return x < xi ? xi - 1 : xi;
}

f64 SimplexNoise::dot(Grad g, f64 x, f64 y) {
	return g.x * x + g.y * y;
}

f64 SimplexNoise::dot(Grad g, f64 x, f64 y, f64 z) {
	return g.x * x + g.y * y + g.z * z;
}

f64 SimplexNoise::dot(Grad g, f64 x, f64 y, f64 z, f64 w) {
