...
bool AABB::intersect(AABB &b) {
	...
	vec3 hsize = getSize();
	vec3 dist = (b.getCenter() - center).abs();
	if (dist.x >= b.getSize().x + hsize.x)return false;
	if (dist.y >= b.getSize().y + hsize.y)return false;
	if (dist.z >= b.getSize().z + hsize.z)return false;
	return true;
}
...
vec3 AABB::getMin() { return min; }
vec3 AABB::getMax() { return max; }
vec3 AABB::getSize() { return (max - min) / 2; }
f32 AABB::maxRadius() { vec3 hsize = getSize();  return hsize.x > hsize.y && hsize.x > hsize.z ? hsize.x : (hsize.y > hsize.x && hsize.y > hsize.z ? hsize.y : hsize.z); }
vec3 AABB::getCenter() { return min + getSize(); }